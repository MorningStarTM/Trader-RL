...
class PPO:
    def __init__(self, config):
        ...
        self.policy = ActorCritic(self.config['input_dim'], self.config['action_dim'], self.config['has_continuous_action_space'], self.config['action_std_init']).to(self.device)
        self.optimizer = torch.optim.Adam([
                        {'params': self.policy.actor.parameters(), 'lr': self.config['lr_actor']},
                        {'params': self.policy.critic.parameters(), 'lr': self.config['lr_critic']}
                    ])

        self.policy_old = ActorCritic(self.config['input_dim'], self.config['action_dim'], self.config['has_continuous_action_space'], self.config['action_std_init']).to(self.device)
